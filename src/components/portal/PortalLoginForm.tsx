...
function PortalLoginFields() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!supabase) {
      setLoading(false);
      setError("Portal sign-in is not configured yet.");
      return;
    }
  ...
  async function signInWithGoogle() {
    ...
    const { error: oauthError } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `${origin}/auth/callback?next=${encodeURIComponent("/portal/dashboard")}`,
      },
    });
    setGoogleLoading(false);
    if (oauthError) {
  ...
  return (
    <div className="mx-auto w-full max-w-md space-y-5 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)]/40 p-8">
      <button
        type="button"
      ...
      <p className="text-center text-xs text-[var(--color-muted)]">
        First time here? Google will create your portal account automatically
        once sign-in is enabled in Supabase.
      </p>
